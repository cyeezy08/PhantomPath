...
![PathFinder Banner](docs/demo-preview.png)
...
> ⚠️ **LEGAL DISCLAIMER** — Read before use. See the [full disclaimer](#legal-disclaimer--responsible-use) at the bottom of this file.
...
PathFinder is an **educational security research tool** that models Active Directory networks as graphs and uses AI to reason over them like a red team operator would.

You build (or import) a network graph — nodes represent hosts, accounts, and services; edges represent relationships like `AdminTo`, `HasSession`, `CanRDP`, `Kerberoastable`. PathFinder then:

1. Runs a **local BFS graph traversal** to compute all attack paths from entry points to high-value targets
2. Sends the graph topology to your chosen **AI model** (Claude, GPT-4o, Gemini, Mistral, etc.) with a red-team-operator system prompt
3. Returns structured threat intelligence: attack path chains with hop-by-hop breakdowns, MITRE ATT&CK technique IDs, privilege escalation opportunities, high-value target rankings, risk ratings, and specific defensive recommendations

This is the kind of analysis a skilled penetration tester does manually when reviewing BloodHound output — PathFinder automates and augments that reasoning process.

**Who is this for?**
- 🔴 **Red teamers** — quickly reason over AD graphs and identify the fastest paths to DA
- 🔵 **Blue teamers** — understand what attackers see and where to place controls
- 🟣 **Security students** — learn AD attack paths in an interactive, visual environment
- 🟡 **Pentesters** — prototype and document attack narratives for reports
- 🔬 **Researchers** — experiment with AI-assisted threat modeling
...
### Web UI
- **Visual graph builder** — drag-and-drop node types onto an interactive canvas
- **Right-click context menus** — connect nodes, mark hosts as compromised, trigger node-level AI analysis
- **AI Threat Intelligence panel** — full structured red team report with color-coded sections
- **Attack path highlighting** — dangerous edges glow red after analysis
- **Quick Queries** — one-click questions like "Full kill chain", "Pivot points", "Blast radius"
- **Freeform chat** — ask anything about the loaded graph in natural language
- **Multi-provider support** — switch between Anthropic, OpenAI, and OpenRouter in Settings
- **Per-provider key storage** — keys saved in localStorage, separate per provider
- **Import / Export** — load and save network graphs as JSON
- **Demo network** — one-click load of a realistic 11-node corporate AD environment
- **Zero dependencies** — single `index.html` file, opens in any modern browser

### CLI
- **BFS path engine** — local graph traversal, no AI required
- **AI analysis** — deep threat intel via any supported provider
- **Multi-provider** — `--provider anthropic|openai|openrouter` flag
- **Model override** — `--model` flag for any model on any provider
- **Interactive mode** — build a graph step-by-step in the terminal
- **Freeform queries** — `--query` flag to ask specific questions
- **JSON output** — `--output` to save full report
- **No pip installs** — uses Python stdlib `urllib` only
...
|---|---|---|
| AI threat analysis | ✅ | ✅ |
| Local BFS path finding | ✅ | ✅ |
| MITRE ATT&CK annotations | ✅ | ✅ |
| Anthropic / OpenAI / OpenRouter | ✅ | ✅ |
| Import/export JSON | ✅ | ✅ |
| Custom freeform queries | ✅ | ✅ |
| Demo corporate AD network | ✅ | ✅ |
| Save reports | ✅ | ✅ |
| Visual graph canvas | ✅ | — |
...
This is the same underlying model used by BloodHound/SharpHound. Relationships between objects in Active Directory translate directly to edges that an attacker can traverse.

### Path Finding

The CLI engine runs **Breadth-First Search** from every compromised or entry-point node, finding all reachable paths to high-value targets (Domain Controllers, Admin accounts, Databases). Each path is scored by the risk level of nodes and the danger of edge types along it.

### AI Reasoning

The graph is serialized to a structured text representation and sent to the AI model with a carefully engineered system prompt:
...
The AI reasons over this topology and returns a structured report divided into five sections: Critical Attack Paths, Privilege Escalation Chains, High-Value Targets, Risk Assessment, and Defensive Recommendations — each referencing specific nodes, edge types, and MITRE technique IDs.
...
Then:
1. Click **⚙️ Settings** → select your AI provider → paste your API key → Save
...
1. **Add nodes** — drag any node type from the left panel onto the canvas
2. **Connect nodes** — right-click a node → "Connect to…" → click the target node → choose an edge type
3. **Mark compromised** — right-click a node → "Mark Compromised" to simulate a breached host
4. **Move nodes** — click and drag any node to reposition it
...
- **Node analysis** — right-click any node → "AI: Analyze this node" for focused threat intel on that specific host
- **Quick Queries** — predefined questions in the right panel for common scenarios
- **Freeform chat** — type any question, e.g. *"How would an attacker pivot from Sam-PC to the database?"*

### Settings

Click **⚙️ [Provider Name]** in the top bar:
- Choose your **AI Provider** (Anthropic / OpenAI / OpenRouter)
- Select a **Model** from the dropdown
- Paste your **API Key** — stored in browser localStorage, sent only to the provider directly
...
| `--no-ai` | Skip AI analysis, run graph engine only (no API key needed) |
...
| `--api-key KEY` | Pass API key directly instead of using environment variable |
...
| Model | Notes |
|---|---|
| `claude-sonnet-4-20250514` | Recommended — fast, highly capable |
...
One API key, 100+ models. Good for comparing outputs across providers.
...
| `db` | Database | 🟡 Medium | SQL/NoSQL server — high data exfiltration value |
...
- Vanilla HTML/CSS/JS — zero build step, zero npm, zero frameworks
- Canvas-based graph renderer via native HTML5 Canvas API
- Direct browser-to-API calls via `fetch()`
- API keys stored in `localStorage` only — never leave your machine except to the provider
- Entire app is one self-contained `index.html`

### CLI Stack

- Python 3.8+ — standard library only (`json`, `urllib`, `collections`, `pathlib`, `argparse`)
...
PathFinder has no server-side component by design. Your network topology and API keys never pass through any third-party server — all requests go directly from your browser or terminal to the AI provider. This matters for security research tooling where the data being analyzed may be sensitive.

---

## Roadmap

- [ ] **BloodHound JSON importer** — parse SharpHound `.json` collection output directly
- [ ] **Neo4j backend** — store and query large AD graphs at scale
- [ ] **CVSS-style path scoring** — numeric risk score per discovered attack path
- [ ] **MITRE ATT&CK heatmap** — visualize technique coverage across the graph
- [ ] **Defense simulation mode** — add a control and see which paths break
- [ ] **Multi-hop Kerberos modeling** — model constrained/unconstrained delegation chains accurately
- [ ] **PDF report export** — formatted penetration testing report output
- [ ] **REST API mode** — expose PathFinder as a backend service
- [ ] **Custom node/edge types** — user-defined relationship types
- [ ] **Graph diff** — compare two network snapshots to track remediation

---

## Inspiration & Prior Art

PathFinder is inspired by and builds conceptually on the following. It is not affiliated with or endorsed by any of them.
...
- New node or edge types with appropriate risk ratings
- Additional MITRE ATT&CK technique mappings
- Improved path scoring heuristics
- UI improvements
- Additional example network JSON files
- Documentation improvements

Please maintain the zero-dependency philosophy for both the web UI and CLI.

---

## Legal Disclaimer & Responsible Use

> **READ THIS SECTION BEFORE USING PATHFINDER.**

### Intended Use

PathFinder is designed and intended **exclusively** for:
...
- **CTF competitions** and training platforms such as HackTheBox, TryHackMe, and similar
...
- Assist in any illegal activity of any kind

### No Warranty

PathFinder is provided **as-is**, with no warranty of any kind, express or implied. The authors make no representations about the accuracy, completeness, or fitness for purpose of the tool or its AI-generated outputs.

**AI-generated threat intelligence is not guaranteed to be accurate.** Analysis produced by PathFinder's AI engine is probabilistic reasoning over a simplified graph model. It may miss attack paths, produce false positives, reference incorrect technique IDs, or generate analysis that does not apply to your specific environment. All AI output must be reviewed and validated by a qualified security professional before being acted upon.

### Responsibility

The authors and contributors of PathFinder accept **no liability** for any damage, loss, legal consequences, or harm resulting from the use or misuse of this tool. By using PathFinder, you agree that:

- You are solely responsible for ensuring your use is lawful in your jurisdiction
- You have obtained all necessary written permissions before analyzing any network
- You understand that unauthorized computer access is a criminal offense in most countries
...
- And equivalent legislation in virtually every other jurisdiction worldwide
